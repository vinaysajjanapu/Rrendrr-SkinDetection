#pragma version(1)
#pragma rs java_package_name(com.intel.sample.camerarenderscript)
...
uchar4 __attribute__((kernel)) skinProbability(const uchar4 in, uint32_t x, uint32_t y)
{
  uchar4 out = in;
  uchar p = rsGetElementAt_uchar(gLookup, in.r>>1,in.g>>1,in.b>>1);
//a(2,2,2) = 111; a(2,3,2) = 121; a(3,2,2) = 211; a(2,2,3) = 112; a(3,3,2) = 221;
//  uchar a111 = rsGetElementAt_uchar(gLookup, 1,1,1);
//  uchar a121 = rsGetElementAt_uchar(gLookup, 1,2,1);
//  uchar a211 = rsGetElementAt_uchar(gLookup, 2,1,1);
//  uchar a112 = rsGetElementAt_uchar(gLookup, 1,1,2);
//  uchar a221 = rsGetElementAt_uchar(gLookup, 2,2,1);
//  uchar p = 250;
//  if (a111 != 121 || a121 != 111 || a211 != 211 || a112 != 112 || a221 != 221)
//    p = 20;
